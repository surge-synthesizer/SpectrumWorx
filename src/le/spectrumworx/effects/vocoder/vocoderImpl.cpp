...
void VocoderImpl::setup(IndexRange const &workingRange, Engine::Setup const &engineSetup)
{
    ...
    case FilterMethod::MovingAverage:
        using Math::convert;
        filterLength_ = std::min<std::uint16_t>(
            workingRange.size() - 1,
            // http://mathforum.org/kb/thread.jspa?forumID=13&threadID=91719&messageID=455475
            convert<std::uint16_t>(0.443f * engineSetup.sampleRate<float>() /
                                   convert<float>(envelopeBorder)));
        break;
...
class RC
{
  protected:
    ...
    static float tau(std::uint16_t const cutoffBin, Engine::Setup const &setup,
                     float const correction = 1)
    {
...
class LogRC : private RC
{
  public:
    ...
    float processForward(float const sample) const
    {
        auto const result(RC::process(sample, currentRelease_));
        currentRelease_ += releaseDelta_;
        return result;
    }
    float processBackward(float const sample) const
    {
        auto const result(RC::process(sample, currentRelease_));
        currentRelease_ -= releaseDelta_;
        return result;
...
void VocoderImpl::process(Engine::MainSideChannelData_AmPh data, Engine::Setup const &setup) const
{
    ...
    else if (filterMethod() == FilterMethod::MelEnvelope)
    {
        ...
        for (float &bin : envelope)
        {
            bin = envelopeCalculator.processForward(bin);
        }
        envelopeCalculator.reset();
    ...
    else if (filterMethod() == FilterMethod::Passthrough)
    {
    }
    else
    {
    ...
    if (parameters().get<NoiseIntensity>()) [[unlikely]]
    {
        ...
        float const noiseLevel(
            std::sqrt(Math::percentage2NormalisedLinear(parameters().get<NoiseIntensity>())));

        float const ampNoiseRange(noiseLevel * setup.maximumAmplitude() / 8);
        for (auto &amp : carrierData.amps())
        {
            amp += Math::rangedRand(ampNoiseRange);
        }
    ...
    multiply(envelope.begin(), data.side().amps().begin(), data.main().amps().begin(),
             data.main().amps().end());
...
void VocoderImpl::lowPassSpectrum_movingAverage(DataRange const &spectrum,
