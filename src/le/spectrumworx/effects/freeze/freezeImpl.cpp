...
void FreezeImpl::process(ChannelState &cs, Engine::ChannelData_AmPh data,
                         Engine::Setup const &) const
{
    ...
    if (!cs.meltDone)
    {
        ...
        else
        {
            ...
            mix(currentMag.begin(), &cs.frozenMagNew[beginBin], currentMag.begin(),
                currentMag.end(), blendFactor);
            mix(currentFreq.begin(), &cs.frozenFreqNew[beginBin], currentFreq.begin(),
                currentFreq.end(), blendFactor);
    ...
    if (!cs.freezeDone)
    {
        ...
        if (cs.normal)
        {
            mix(&cs.frozenMagNew[beginBin], currentMag.begin(), currentMag.begin(),
                currentMag.end(), blendFactor);
            mix(&cs.frozenFreqNew[beginBin], currentFreq.begin(), currentFreq.begin(),
                currentFreq.end(), blendFactor);
        ...
        else
        {
            ...
            mix(&cs.frozenMagNew[beginBin], &cs.frozenMagOld[beginBin], currentMag.begin(),
                currentMag.end(), blendFactor);
            mix(&cs.frozenFreqNew[beginBin], &cs.frozenFreqOld[beginBin], currentFreq.begin(),
                currentFreq.end(), blendFactor);
