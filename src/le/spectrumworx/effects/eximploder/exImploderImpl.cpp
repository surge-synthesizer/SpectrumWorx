                             ...
                             bool const thresholdIsLowerBound) const
{
    ...
    for (float *LE_RESTRICT pAccumMagn(cs.accumMagn.begin()),
         ...
         ++pAccumMagn, ++pAccumFreq, ++pCurrentAmplitude, ++pCurrentFreq)
    {
        ...
        if ((*pCurrentAmplitude > *pAccumMagn) || // if current amp > amp in accumulator
        ...
        else if ( // bin within frequency range
            (pCurrentAmplitude >= data.amps().begin()) && (pCurrentAmplitude < data.amps().end()))
...
{
    float const frameTime(Detail::ExImPloderImpl::setup(parameters().get<Gliss>(),
                                                        parameters().get<Threshold>(),
                                                        parameters().get<Gate>(), engineSetup));

    // Transfer decay time from 0 to -120 dB to frame decay constant:
    float const decay(Math::exp((frameTime / parameters().get<Decay>()) *
...
void PVImploderImpl::process(ChannelState &cs, Engine::ChannelData_AmPh data,
...
void PVExploderImpl::setup(IndexRange const &, Engine::Setup const &engineSetup)
{
    ...
    float const growth(2.0f - Math::exp((frameTime / parameters().get<Growth>()) *
                                        Math::ln(Math::dB2NormalisedLinear(-120))));
...
void PVExploderImpl::process(ChannelState &cs, Engine::ChannelData_AmPh data,
                             Engine::Setup const &) const
{
