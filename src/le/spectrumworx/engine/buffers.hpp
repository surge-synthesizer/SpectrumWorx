...
template <class Data> class DataPairImpl : public DataPair
{
  ...
  public:
    ...
    ReadOnlyDataRange jointView() const
    {
        return ReadOnlyDataRange(first().begin(), second().end());
    }
  ...
  private:
    template <class FullData> DataArray makeData(FullData &fullData, IndexRange const &workingRange)
    {
...
template <class Data> class SharedStorageDataPairImpl : public DataPairImpl<Data>
{
  private:
#ifndef __APPLE__
    //...mrmlj...seems to give slightly better performance out of our FFT...
    static std::uint8_t const partial_address_aliasing_workaround_offset =
        1 * Utility::Constants::vectorAlignment;
#else
...
class SubRange : public DataPairImpl<SubRangeHolder>
{
  public:
    ...
    SubRange(Parent &fullData, IndexRange const &workingRange)
        : DataPairImpl<SubRangeHolder>(fullData, workingRange), pFullData_(&fullData)
    {
    }

    //...mrmlj...these will not work for MainSide-subranges (?reinvestigate and document why?)...
    size_type beginBin() const
    {
        return static_cast<size_type>(first().begin() - full().first().begin());
    }
    size_type endBin() const
    {
        return static_cast<size_type>(first().end() - full().first().begin());
    }
...
class MainSide<SubRange<FullRangeData, SubRangeHolder>>
    : public SubRange<FullRangeData, SubRangeHolder>
{
  public:
